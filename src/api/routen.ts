import { angemeldetAls, fehler, json } from "../lib/antwort";
...
async function uebersicht(env: Env): Promise<Response> {
  const gewerke = await env.DB.prepare(
    `SELECT gewerk,
            COUNT(*)                                                   AS dokumente,
            SUM(CASE WHEN letzter_status = 'fehler' THEN 1 ELSE 0 END) AS fehler,
            SUM(CASE WHEN aktuelle_version_id IS NULL THEN 1 ELSE 0 END) AS ohne_inhalt,
            MAX(letzte_pruefung)                                       AS letzte_pruefung
       FROM dokumente
      GROUP BY gewerk
      ORDER BY gewerk`,
  ).all();
...
async function hochladen(env: Env, request: Request): Promise<Response> {
  ...
  const wer = angemeldetAls(request);
  const zeit = stempel();
  ...
  await env.DB.batch([
    env.DB.prepare(
      `INSERT INTO versionen
         (id, dokument_id, erfasst_am, r2_raw_key, r2_md_key, bytes, etag, hochgeladen_von)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  return json({ ok: true, dokument_id: dokumentId, version_id: versionId });
}
